...
benchmark_ls() {
    ...
    while IFS= read -r -d '' aider_json_file; do
        (( i+=1 ))
        read tokens_sent tokens_received <<< "$(
          cat "$(dirname "$aider_json_file")"/.aider.chat.history.md | \
          extract_token_counts
        )"
        ((total_tokens_sent+=tokens_sent, total_tokens_received+=tokens_received))
        ...
        read test_cost test_duration test_timeouts test_num_error_outputs test_num_user_asks test_num_exhausted_context_windows test_num_malformed_responses test_syntax_errors test_indentation_errors test_lazy_comments \
    ...
    printf '# duration_s, test_pass_count, test_failed_count, failed_attempts, total_tokens_sent, total_tokens_received,total_cost, total_test_timeouts, total_num_error_outputs, total_num_user_asks, total_num_exhausted_context_windows, total_num_malformed_responses, total_syntax_errors, total_indentation_errors, total_lazy_comments\n'
